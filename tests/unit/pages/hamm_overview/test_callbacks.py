...
class TestUpdateDashboardCallback:
    def test_update_dashboard_returns_14_outputs(self, monkeypatch):
        ...
        monkeypatch.setattr(cb_mod, "prepare_language_display_df", lambda df: pd.DataFrame({"Task ID": ["1"]}))
        ...
        monkeypatch.setattr(cb_mod, "build_language_table", lambda df: ("Language Details", "lang_table"))
...
# ---------------------------------------------------------------------------
# Language Table callback integration (RED -- not yet implemented)
# ---------------------------------------------------------------------------

class TestUpdateDashboardLanguageTable:
    """update_dashboard must include CHART_ID_LANGUAGE_TABLE in its Outputs."""
    ...
    def test_output_includes_language_table_id(self):
        ...
        # Inspect the module source to verify the Output is declared
        import inspect
    ...
    def test_update_dashboard_returns_14_outputs(self, monkeypatch):
        """After adding language table, update_dashboard must return 14 outputs."""
        import plotly.graph_objects as go
        import src.pages.hamm_overview._callbacks as cb_mod
        ...
        monkeypatch.setattr(cb_mod, "build_volume_chart", lambda df: go.Figure())
        monkeypatch.setattr(cb_mod, "build_task_table", lambda df: ("Task Details", "task_table"))
        monkeypatch.setattr(cb_mod, "build_language_table", lambda df: ("Language per Task", "lang_table"))
        monkeypatch.setattr(cb_mod, "build_error_ratio_chart", lambda df: go.Figure())
        monkeypatch.setattr(cb_mod, "build_error_by_screener_chart", lambda df: go.Figure())
        monkeypatch.setattr(cb_mod, "build_user_breakdown_chart", lambda df: go.Figure())
        monkeypatch.setattr(cb_mod, "build_hamm_breakdown_chart", lambda df: go.Figure())
    ...
    def test_update_dashboard_error_path_returns_14_outputs(self, monkeypatch):
        """Error path must also return 14-element tuple after language table addition."""
        import src.pages.hamm_overview._callbacks as cb_mod
