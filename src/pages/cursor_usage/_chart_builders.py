...
def build_daily_cost_trend(df: pd.DataFrame):
    ...
    return build_chart(daily_cost, COST_TREND_SPEC)
...
def build_token_efficiency_chart(df: pd.DataFrame):
    ...
    return build_chart(model_stats, TOKEN_EFFICIENCY_SPEC)
...
def build_model_distribution_chart(df: pd.DataFrame):
    ...
    return build_chart(model_dist, MODEL_DISTRIBUTION_SPEC)
